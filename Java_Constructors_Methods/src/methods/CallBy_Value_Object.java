...
//	   2. Call By Reference (or Object) : Passing a reference of the object   
//							(i.e the variable through which the object can be accessed).
//							 -> It changes the actual value.
//
//	The parameters passed to method are called Actual parameters and the parameters
//	received by method are called Formal parameters.
//
//	In case of Call by value, values of actual parameter are copied to the formal 
//	parameter where both the parameters are stored in different memory locations.  
//	So, any changes made inside the method being called are not reflected in actual 
//	parameters of the caller method.
//
//	But, for Call by reference, both actual and formal parameters refer to the same
//	memory location. So, any changes made inside the method being called are 
